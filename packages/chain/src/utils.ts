...
export async function getTxResult(
  ...
  try {
    // await to handle error
    return await defaultGetTxResult(chain, TransactionId, reGetCount, notExistedReGetCount);
  } catch (error: any) {
    throw new ProviderError(error.message, ResponseCode.ERROR_IN_PARAMS);
  }
}
