...
Use it to detect if current Portkey Extension's wallet is connected.

```typescript
if(!provider.isConnected()){
  // Portkey Extension's wallet is not connected
  alert("it seems that we have lost the Extension's wallet connection");
...
on(event,callback) is used to listen to the events triggered by Portkey Extension.
once(event,callback) works in a similar way, but it will only be triggered once.  

```typescript
provider.on('connected',()=>{
  // Portkey Extension's wallet is connected
  ...
});
provider.once('accountsChanged',(accounts)=>{
  // Portkey Extension's wallet's accounts have changed
  ...
});
```
...
You can use it to determine which chain the Extension are using.

### __3. `chainId : ChainId`__

if `type` is `'aelf'` , `chainId` shows which chain type the Extension is on.  
...
### __1. `method:'chainId'` / `method:'chainIds'`__
  ...
  Returns the current chainId of the Portkey Extension's wallet.  
  ...
  provider.request({ method: "chainIds" }).then((chainIds: ChainId[]) => {
    console.log('current Extension supports:',chainIds);
  });
  ```
...
### __3. `method:'requestAccounts'`__
  ...
  Request the Portkey Extension's wallet to connect to your dapp, this method is the bridge to get the permission required by the following methods below.  
  If the user has not connected to your dapp, the Portkey Extension's wallet will pop up a window to ask the user to connect to your dapp.  
...
### __4. `method:'accounts'`__
  ...
  Returns the current account addresses of the Portkey Extension's wallet.  
...
### __5. `method:'wallet_getWalletState'`__
  ...
  Returns the current wallet state of the Portkey Extension's wallet.  
  __NOTICE__: You should use `request({ method: 'requestAccounts' })` first for the permission to access.
...
### __6. `method:'wallet_getWalletName'`__
  ...
  Returns the current wallet name of the Portkey Extension's wallet.  
  __NOTICE__: You should use `request({ method: 'requestAccounts' })` first for the permission to access.
...
### __7. `{method:'sendTransaction',payload:SendTransactionParams}`__
  ...
  Send a transaction to the Portkey Extension's wallet.  
  __NOTICE__: You should use `request({ method: 'requestAccounts' })` first for the permission to access.  
